...
router.get("/sendcodewithname", async function (req, res) {
  ...
  const { status: userStatus, rows: userRows } = await crud.getDataListFromTable('', 'UserMst', { UserName: userName, UserPhone: userPhone , UserEmail: userEmail })
  if (userStatus === -1) {
...
async function send_auth_code(phone, res) {
  ...
  const uri = "ncp:sms:kr:295405573609:hondev"; //서비스 ID
  const secretKey = "dwyodnBSNLTfEzxh6rAMXe3XZisROAmmHEwdotTZ"; // Secret Key
  const accessKey = "eAaWhx4YBud6o3mqlkYZ"; //Access Key
  const method = "POST";
  const space = " ";
  const newLine = "\n";
  ...
  await axios({
    ...
    data: {
      ...
      countryCode: "82",
      from: "01082487509",
      content: `테스트문자-인증번호는[${authNumber}]입니다.`,
